...
interface LeafAttributes extends HTMLElement {
...
export const Editable = (props: EditableProps) => {
  const defaultRenderPlaceholder = (props: RenderPlaceholderProps) => (
  ...
  const [deferredOperations, setDeferredOperations] = createSignal<
    DeferredOperation[]
  >([]);
  ...
  createRenderEffect(
    on([reactiveUpdates], () => EDITOR_TO_FORCE_RENDER.set(editor, forceRender))
  );

  // Update internal state on each render.
  createRenderEffect(
    on([reactiveUpdates], () => IS_READ_ONLY.set(editor, merged.readOnly))
  );
   ...
   */
  let androidInputManagerRef: AndroidInputManager | null | undefined =
    undefined;
  ...
  const onDOMSelectionChange = createMemo(
    on(
      [
        reactiveUpdates,
        // () => merged.reactive.children,
        // () => merged.reactive.selection,
        () => merged.readOnly,
        state,
      ],
      () =>
        throttle(() => {
          ...
          if (
            ...
            const anchorNodeSelectable =
              ...
              SolidEditor.isTargetInsideNonReadonlyVoid(
                editor,
  ...
  createEffect(
    on([reactiveUpdates], () => {
      ...
      const root = SolidEditor.findDocumentOrShadowRoot(editor);
      const domSelection = root.getSelection();
      console.log("Setting Selection Manually: ", domSelection?.toString());
      ...
      const setDomSelection = (forceChange?: boolean) => {
        ...
        if (
          editor.selection &&
          !SolidEditor.hasRange(editor, editor.selection)
        ) {
          // Beware change it
  ...
  const onDOMBeforeInput = (event: InputEvent) => {
    ...
    if (
      ...
      if (
        ...
        const [node, offset] = SolidEditor.toDOMPoint(
          editor,
  ...
  return (
    <ReadOnlyContext.Provider value={getReadOnly}>
      <DecorateContext.Provider value={getDecorate}>
        <RestoreDOM
          node={ref}
          receivedUserInput={userInputData().receivedUserInput()}
        >
          <Dynamic
              ...
              if (SolidEditor.hasSelectableTarget(editor, event.target)) {
                ...
                if (editor.selection) {
                  ...
                  if (inline) {
                    ...
                    if (
                      Editor.isEnd(editor, editor.selection.anchor, inlinePath)
                    ) {
