import { Editor, Node, Operation, Path, Point, Range, Text, Element } from "slate";
...
function longestCommonSuffixLength(str: string, another: string, max: number): number {
...
export function normalizeStringDiff(targetText: string, diff: StringDiff) {
	...
	const max = Math.min(removedText.length - prefixLength, text.length - prefixLength);
 ...
 * both changes
 */
export function mergeStringDiffs(targetText: string, a: StringDiff, b: StringDiff): StringDiff | null {
	const start = Math.min(a.start, b.start);
	const overlap = Math.max(0, Math.min(a.start + a.text.length, b.end) - b.start);
...
export function transformPendingRange(editor: Editor, range: Range, op: Operation): Range | null {
...
export function transformTextDiff(textDiff: TextDiff, op: Operation): TextDiff | null {
