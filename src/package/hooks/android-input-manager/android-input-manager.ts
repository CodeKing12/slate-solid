...
export function createAndroidInputManager(
  props: CreateAndroidInputManagerOptions,
): AndroidInputManager {
  ...
  const performAction = () => {
    ...
    if (action.at) {
      ...
      if (
        ...
        !Range.equals(props.editor.selection, targetRange)
      ) {
  ...
  const flush = () => {
    ...
    const selectionRef =
      props.editor.selection &&
      Editor.rangeRef(props.editor, props.editor.selection, {
        affinity: "forward",
      });
    ...
    debug(
      "flush",
      EDITOR_TO_PENDING_ACTION.get(props.editor),
      EDITOR_TO_PENDING_DIFFS.get(props.editor),
    );
    ...
    while ((diff = EDITOR_TO_PENDING_DIFFS.get(props.editor)?.[0])) {
      ...
      if (
        !props.editor.selection ||
        !Range.equals(props.editor.selection, range)
      ) {
      ...
      EDITOR_TO_PENDING_DIFFS.set(
        props.editor,
        EDITOR_TO_PENDING_DIFFS.get(props.editor)?.filter(
          ({ id }) => id !== diff!.id,
        )!,
      );
    ...
    if (
      ...
      (!props.editor.selection ||
        !Range.equals(selection, props.editor.selection))
    ) {
  ...
  const handleDOMBeforeInput = (event: InputEvent): void => {
    ...
    if (type.startsWith("delete")) {
      ...
      if (Range.isExpanded(targetRange)) {
        ...
        return scheduleAction(
          () => Editor.deleteFragment(props.editor, { direction }),
          { at: targetRange },
        );
      }
    }
    ...
    switch (type) {
      ...
      case "deleteHardLineBackward": {
        return scheduleAction(
          () => Editor.deleteBackward(props.editor, { unit: "block" }),
          {
            at: targetRange,
          },
        );
      ...
      case "deleteHardLineForward": {
        return scheduleAction(
          () => Editor.deleteForward(props.editor, { unit: "block" }),
          { at: targetRange },
        );
      }
      ...
      case "deleteWordBackward": {
        return scheduleAction(
          () => Editor.deleteBackward(props.editor, { unit: "word" }),
          { at: targetRange },
        );
      }
      ...
      case "deleteWordForward": {
        return scheduleAction(
          () => Editor.deleteForward(props.editor, { unit: "word" }),
          { at: targetRange },
        );
      }
      ...
      case "insertText": {
        if (isDataTransfer(data)) {
          return scheduleAction(
            () => SolidEditor.insertData(props.editor, data),
            {
              at: targetRange,
            },
          );
  ...
  const handleDomMutations = (mutations: MutationRecord[]) => {
    ...
    if (
      mutations.some((mutation) =>
        isTrackedMutation(props.editor, mutation, mutations),
      )
    ) {
