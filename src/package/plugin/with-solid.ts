import { BaseEditor, Editor, Element, Node, Operation, Path, PathRef, Point, Range, Transforms } from "slate";
import { TextDiff, transformPendingPoint, transformPendingRange, transformTextDiff } from "../utils/diff-text";
...
export const withSolid = <T extends BaseEditor>(
	...
	e.addMark = (key, value) => {
		...
		if (!EDITOR_TO_PENDING_INSERTION_MARKS.get(e) && EDITOR_TO_PENDING_DIFFS.get(e)?.length) {
	...
	e.deleteBackward = (unit) => {
		...
		if (e.selection && Range.isCollapsed(e.selection)) {
			...
			if (parentBlockEntry) {
				...
				const parentElementRange = Editor.range(e, parentBlockPath, e.selection.anchor);
	...
	e.apply = (op: Operation) => {
		...
		for (const [pathRef, key] of pathRefMatches) {
			if (pathRef.current) {
				...
				console.log("Setting Node_to_keys 2 -- ", node, key);
				NODE_TO_KEY.set(node, key);
			}
		}
	};
...
const getMatches = (e: SolidEditor, path: Path) => {
	const matches: [Path, Key][] = [];
