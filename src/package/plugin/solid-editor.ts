import { BaseEditor, Editor, Element, Node, Path, Point, Range, Scrubber, Transforms } from "slate";
...
export interface SolidEditor extends BaseEditor {
	hasEditableTarget: (editor: SolidEditor, target: EventTarget | null) => target is DOMNode;
	hasRange: (editor: SolidEditor, range: Range) => boolean;
	hasSelectableTarget: (editor: SolidEditor, target: EventTarget | null) => boolean;
	hasTarget: (editor: SolidEditor, target: EventTarget | null) => target is DOMNode;
	insertData: (data: DataTransfer) => void;
	insertFragmentData: (data: DataTransfer) => boolean;
	insertTextData: (data: DataTransfer) => boolean;
	isTargetInsideNonReadonlyVoid: (editor: SolidEditor, target: EventTarget | null) => boolean;
...
export interface SolidEditorInterface {
	 ...
	 */
	hasDOMNode: (editor: SolidEditor, target: DOMNode, options?: { editable?: boolean }) => boolean;
	 ...
	 */
	insertData: (editor: SolidEditor, data: DataTransfer | null) => void;
	 ...
	 */
	setFragmentData: (editor: SolidEditor, data: DataTransfer | null, originEvent?: "drag" | "copy" | "cut") => void;
...
export const SolidEditor: SolidEditorInterface = {
	...
	hasEditableTarget: (editor, target): target is DOMNode =>
		isDOMNode(target) && SolidEditor.hasDOMNode(editor, target, { editable: true }),
	...
	hasSelectableTarget: (editor, target) =>
		SolidEditor.hasEditableTarget(editor, target) || SolidEditor.isTargetInsideNonReadonlyVoid(editor, target),

	hasTarget: (editor, target): target is DOMNode => isDOMNode(target) && SolidEditor.hasDOMNode(editor, target),
	...
	toDOMRange: (editor, range) => {
		...
		const domFocus = Range.isCollapsed(range) ? domAnchor : SolidEditor.toDOMPoint(editor, focus);
	...
	toSlatePoint: <T extends boolean>(
		...
		if (IS_ANDROID && !textNode && !exactMatch) {
			...
			if (node && SolidEditor.hasDOMNode(editor, node, { editable: true })) {
				...
				let { path, offset } = Editor.start(editor, SolidEditor.findPath(editor, slateNode));
	...
	toSlateRange: <T extends boolean>(
			...
			if (isDOMSelection(domRange)) {
				...
				if (IS_FIREFOX && domRange.rangeCount > 1) {
					...
					if (
						...
						const firstNode = getLastChildren(<HTMLElement>firstNodeRow.children[firstRange.startOffset]);
						const lastNode = getLastChildren(<HTMLElement>lastNodeRow.children[lastRange.startOffset]);
				...
				if ((IS_CHROME && hasShadowRoot(anchorNode)) || IS_FIREFOX) {
					isCollapsed =
						domRange.anchorNode === domRange.focusNode && domRange.anchorOffset === domRange.focusOffset;
				} else {
		...
		const anchor = SolidEditor.toSlatePoint(editor, [anchorNode, anchorOffset], {
			exactMatch,
			suppressThrow,
		});
		if (!anchor) {
